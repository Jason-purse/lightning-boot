...
class JpaDbTemplate(private val context: JpaContext) : DBTemplate {
    ...
    override fun <T : Entity> selectById(query: IDQuerySupport, entityClass: Class<T>): T? {
        ...
        // 强制判断 ..
        Assert.isTrue(idType == idQuery.getIdClass(), "id type must be equals !!!")
        return getRepository(entityClass).findById(idQuery.getQueryInfo().getNativeQuery()).orElse(null)
    }
